...
public class JeoDatasetDescribeResource extends SitoolsParameterizedResource {
  ...
  /**
   * Handle the Exception and create a representation from the message of the exception
   * 
   * @param e
   *          the Exception
   * @param variant
  ...
  /**
   * Get the Dataset description Representation stored, if there is no description stored it throws a SitoolsException
   * 
   * @return a Representation
    ...
    if (dicoName != null) {
      ...
      if (dico != null) {
        ...
        DatabaseRequestParameters databaseParams = new DatabaseRequestParameters(application.getDataSource(), 0, false,
            true, ds, ds.getPredicat(), ds.getStructures(), null, null, ds.getColumnModel(), 0, null, null, null,
            ds.getColumnModel());
  ...
  private Filter createFilter(String colAlias, FilterType type, Concept concept, boolean unique, int totalCount)
    ...
    switch (type) {
      case enumeration:
        ...
        ResultSet rs = ((SitoolsSQLDataSource) application.getDataSource()).basicQuery(strReq, 0, 0);
  ...
  /**
   * Encode a JeoDescribe into a Representation according to the given media type.
   * 
   * @param describe
