...
public class JeoInsertRecordResource extends SitoolsParameterizedResource {
  ...
  public Representation insert(Representation representation, Variant variant) {
    ...
    try {
      con = SitoolsSQLDataSourceFactory.getInstance()
          .getDataSource(dataset.getDatasource().getId()).getConnection();
      ...
      if (stm.getWarnings() == null) {
        ...
        response = new Response(false, "Created with warnings : "
            + stm.getWarnings());
        getResponse().setStatus(Status.SUCCESS_CREATED);
      }
  ...
  private String createSQLInsert(Structure struct, Record record) {
    ...
    for (Iterator<AttributeValue> it = attr.iterator(); it.hasNext();) {
      ...
      if (col != null) {
        ...
        if (columnAliasColGeometry != null
            && attrValue.getName().equals(columnAliasColGeometry)) {
   ...
   */
  private String createSQLUpdate(Structure struct, Record record,
      DataSet dataset) {
  ...
  public final Record getObject(Representation representation) {
    ...
    if (MediaType.APPLICATION_XML.isCompatible(representation.getMediaType())) {
      ...
      object = new JacksonRepresentation<Record>(representation, Record.class)
          .getObject();
    }

    else if (MediaType.APPLICATION_JAVA_OBJECT.isCompatible(representation
        .getMediaType())) {
  ...
  private boolean checkRecordExists(DataSet dataset, Record record) {
    Request reqGET = new Request(Method.GET, RIAPUtils.getRiapBase()
        + dataset.getSitoolsAttachementForUsers() + "/records/"
        + record.getId());
  ...
  private Column getPrimaryKey(DataSet dataset) {
    ...
    Column pkKey = null;
    for (Iterator<Column> iterator = col.iterator(); iterator.hasNext()
        && pkKey == null;) {
